...
impl Contract {
    ...
    pub fn create_task(
        ...
        if min_balance_needed > item.total_deposit.0 {
            panic!(
                "Not enough task balance to execute job, need at least {}, attached {}",
    ...
    pub fn test_task(
        ...
        let contract_id: ValidAccountId = "counter_test.testnet".to_string().try_into().unwrap();
        let function_id = String::from("increment");
        let recurring: Option<bool> = Some(false);
        let gas: Option<Gas> = Some(1);
        let deposit = U128(500000000000000000000000);
        let cadence = String::from("28 14 5 4 * *");
        ...
        return self.create_task(
            ...
            //    (*serde_json::to_string(&arguments).unwrap().as_bytes()).to_vec(),
            // )),
        );
    }
